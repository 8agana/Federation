"""
Natural language time expression parser for CC-DT Memory Federation.

Adapted from MCP Memory Service for our memory system.
Handles queries like "yesterday", "last week", "3 days ago", "yesterday afternoon".
"""
import re
import logging
...
def get_last_period_range(period: str) -> Tuple[float, float]:
    ...
    if period == "day":
        ...
        last_of_last_month = date(today.year, today.month, 1) - timedelta(days=1)
...
def extract_time_from_query(query: str) -> Dict[str, Any]:
    """
    Extract time-related expressions from a query and return both cleaned query and timestamps.
    ...
    Returns:
        Dict with:
            - cleaned_query: Query with time expressions removed
            - start_timestamp: Start of time range (or None)
            - end_timestamp: End of time range (or None)
            - time_expression: The matched time expression (or None)
    """
    ...
    time_expression = query[matches[0].start():matches[0].end()]
    
    # Parse time expressions to get timestamps
    start_ts, end_ts = parse_time_expression(time_expression)
    
    # Remove time expressions from the query
    cleaned_query = query[:matches[0].start()] + query[matches[0].end():]
